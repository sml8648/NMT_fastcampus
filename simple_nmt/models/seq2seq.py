...
class Decoder(nn.Module):
    ...
    def forward(self, emb_t, h_t_l_tilde, h_t_l):

        batch_size = emb_t.size(0)
        hidden_size = h_t_l[0].size(-1)

        if h_t_l_tilde is None:
            h_t_l_tilde = emb_t.new(batch_size, 1, hidden_size).zero_()

        x = torch.cat([emb_t, h_t_l_tilde], dim=-1)

        y, h = self.rnn(x, h_t_l)
