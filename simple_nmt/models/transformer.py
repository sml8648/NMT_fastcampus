...
import simple_nmt.data_loader as dataloader
...
class MultiHead(nn.Module):
    ...
    def forward(self, Q, K, V, mask=None):
        ...
        KWs = self.K_lianer(K).split(self.hidden_size // self.n_splits, dim=-1)
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, key_and_value, mask, prev, future_mask):
        ...
        z = z + self.attn_dropout(self.attn(Q=self.attn_norm(z),
                                            ...
                                            mask=mask))
        z = self.fc_form(z + self.fc_dropout(self.fc(z)))
...
class Transformer(nn.Module):
    ...
    def _generate_mask(self, x, length):
        ...
        for l in length:
            if max_length - 1 > 0:
                ...
                mask += [torch.cat([x.new_ones(1,1).zero_(),
    ...
    def forward(self, x, y):
        ...
        with torch.no_grad():
            ...
            future_mask = torch.triu(x.new_ones((y_size(1), y_size(1))), diagonal=1).bool()
    ...
    def search(self,x,is_greedy=True, max_length=255):
        ...
        while is_decoding.sum() > 0 and len(indice) < max_length:
            ...
            if is_greedy:
                ...
                y_t_1 = torch.multinomial(y_hat_t.exp().view(x_size(0), -1),1)
    ...
    def batch_beam_search(
        ...
        done_cut = [board.is_done() for board in boards]

        length = 0
        while sum(done_cut) < batch_size and length <= max_length:
            ...
            for i, board in enumerate(boards):
                if board.is_done() == 0:
                    ...
                    fab_input += [y_hat_i]
                    fab_z += [z[i].unsqueeze(0)]
                    fab_mask == [mask_dec[i].unsqueeze(0)]
        ...
        for i, board in enumerate(boards):
            sentences, probs = borad.get_n_best(n_best, length_penalty=length_penalty)
