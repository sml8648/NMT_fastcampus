...
class MaximumLikelihoodEstimationEngine(Engine):
    ...
    def train(engine, mini_batch):
        ...
        mini_batch.src = (mini_batch.tgt[0].to(device), mini_batch.tgt[1])
        ...
        if engine.state.iteration % engine.config.iteration_per_update == 0:
            ...
            if engine.config.user_noam_decay and engine.lr_scheduler is not None:
    ...
    def validate(engine, mini_batch):
        ...
        word_count = int(mini_bach.tgt[1].sum())
        loss = flaot(loss / word_count)
        ppl = np.exp(loss)
    ...
    def attach(
        ...
        for metric_name in validation_metric_names:
            attach_running_averag(validation_engine, metric_name)
...
class SingleTraniner():
    ...
    def train(
        ...
        def run_validation(engine, validation_engine, valid_loader):
            ...
            if engine.lr_scheduler is not None and not engine.config.user_noam_decay:
                engine.lr_scheduler.step()
